...
The confirmation is a decision with a record, made by a named Human, that the work meets the definition of done as amended and may be relied on within a defined scope. It is the moment Final Liability attaches by choice rather than by accident. Everything else in this repository exists to make this moment honest: the definition gives it a standard, the Salience Audit gives it eyes and the four lenses give it depth.

The confirmation is not review, not consensus and not the absence of objection. It is one Human, one artifact, one standard, one signature and one scope.
...
Refusal is different. Refusal requires no independence and no seniority. Anyone in the role chain who finds a failed test may refuse and return the work, at any tier, at any time. Confirmation is what requires the designated Human. A workflow in which refusing is hard and confirming is easy has been built backwards.
...
**Confirm.** The definition is met. The named Human signs, the scope is stated and the work is released within it.
...
## Confirming under pressure

Pressure does not change the protocol. It changes the honesty required to run it. The five conditions of a valid authorization from Workbook Module 10 apply to the confirmation as written: knowledge, evidence, authority, time and permission. Two fail most often under pressure.

**Time.** A confirmation demanded in the gap between the output landing and the meeting starting is reflexive, not considered. The architecture discipline applies: the record shows when the material was available, when the decision was made and what happened in between. If the time was not there, the confirmation was not either, whatever the signature says.

**Permission.** A Human who cannot refuse in practice cannot confirm in substance. An organization that punishes refusals has no confirmations, only ratifications, and its signature records are an inventory of future defendants rather than evidence of governance. Permission is built by how the last refusal was received. Every leader gets the confirmation culture their reaction to bad news has purchased.

The remaining condition worth naming is authority: the confirmer must hold the institutional power to bind the organization to reliance on this work, at this tier. Confirmation delegated below the level of meaningful comprehension is a design failure, not an efficiency.

## What the confirmation is for

A reviewer who was not present, arriving years later with the file, should be able to reconstruct what was defined, what was tested, what was found, what was unknown, what was traded and who decided, and conclude that a specific Human made a knowing decision. That reconstruction is the entire value of the record. The work product will age and the tools will be replaced. The evidence that a Human defined done and confirmed it knowingly is what stands between an organization and the claim that nobody decided anything.
